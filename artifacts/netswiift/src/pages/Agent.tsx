...
function DepositModal({ onClose, onDeposited }: { onClose: () => void; onDeposited: (amount: number) => void }) {
  ...
  function handleDeposit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem("nsWallet", JSON.stringify({ balance: current + numAmount }));
      setLoading(false);
      setDone(true);
            ...
            {done ? (
              <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">GHS {numAmount.toFixed(2)} added to your wallet</p>
                </div>
              ...
              <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} onSubmit={handleDeposit} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={amount} onChange={e => setAmount(e.target.value)} placeholder="e.g. 50" type="number" min="1" inputMode="decimal"
                    className="h-14 text-2xl font-bold bg-black/20 border-white/10 focus-visible:ring-teal-500 text-white text-center" autoFocus />
                </div>
                ...
                {numAmount > 0 && (
                  <div className="bg-black/20 rounded-xl p-4 border border-white/5 space-y-2 text-sm">
                    <div className="flex justify-between text-muted-foreground"><span>Amount</span><span className="text-white">GHS {numAmount.toFixed(2)}</span></div>
                    <div className="flex justify-between text-muted-foreground"><span>Paystack fee (3%)</span><span className="text-white">GHS {fee.toFixed(2)}</span></div>
                    <div className="flex justify-between font-bold border-t border-white/5 pt-2"><span className="text-muted-foreground">You pay</span><span className="text-teal-400 text-base">GHS {total.toFixed(2)}</span></div>
                  </div>
                )}
...
export default function Agent() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-b border-white/5">
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-blue-500/10 border border-blue-500/20">
          <Avatar className="h-9 w-9 border border-blue-500/30">
            <AvatarFallback className="bg-gradient-to-br from-blue-500/20 to-teal-600/20 text-blue-400 text-sm font-bold">{initials}</AvatarFallback>
          </Avatar>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <p className="font-semibold text-sm text-white truncate">{user.name}</p>
              <Badge className="text-[10px] border bg-blue-500/10 text-blue-400 border-blue-500/20 shrink-0">Agent</Badge>
            </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        <Link href="/agent" className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gradient-to-r from-teal-500/20 to-purple-500/10 text-teal-400 font-medium text-sm border border-teal-500/20">
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <main className="flex-1 md:pl-64 flex flex-col min-h-screen">
        <header className="sticky top-0 z-40 border-b border-white/5 bg-background/80 backdrop-blur-xl">
          <div className="flex items-center justify-between h-14 px-4 md:px-6">
            ...
            <div className="flex items-center gap-2">
              <Badge className="text-xs border bg-blue-500/10 text-blue-400 border-blue-500/20 hidden sm:flex items-center gap-1">
              ...
              <Avatar className="h-8 w-8 border border-blue-500/30 cursor-pointer">
                <AvatarFallback className="bg-gradient-to-br from-blue-500/20 to-teal-600/20 text-blue-400 text-xs font-bold">{initials}</AvatarFallback>
              </Avatar>
            </div>
          </div>
        </header>
                ...
                <div className="relative p-6 flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-teal-400">
                      <ShieldCheck className="w-3.5 h-3.5" /><span>Secured · Agent Account</span>
                    </div>
                  </div>
              ...
              <div className="divide-y divide-white/5">
                {ordersLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 px-6 text-center space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground">Start buying data for your customers.</p>
                  </div>
                ) : (
                  recentOrders?.map((order, i) => {
                    const net = NETWORKS.find(n => n.id.toLowerCase() === order.network.toLowerCase()) ?? NETWORKS[0];
                    ...
                    return (
                      <motion.div key={order.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.42 + i * 0.04 }}
                        ...
                        <div className={`w-10 h-10 rounded-xl ${net.color} flex items-center justify-center text-white font-bold text-sm shrink-0`}>{net.name[0]}</div>
                        <div className="flex-1 min-w-0">
                          <p className="font-mono text-sm text-white font-semibold truncate">{order.orderId}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{order.network} · {order.bundleSize}</p>
                        </div>
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          <Badge className={`text-xs border ${sc.cls}`}>{order.status}</Badge>
      ...
      <AnimatePresence>
        {showDeposit && (
          <DepositModal onClose={() => setShowDeposit(false)} onDeposited={amt => { setWalletBalance(b => b + amt); setShowDeposit(false); }} />
        )}
      </AnimatePresence>
    </div>
  );
